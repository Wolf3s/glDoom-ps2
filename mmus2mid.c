 ...
 *-----------------------------------------------------------------------------
 *
 * Modified by Pedro Gimeno on 2015-11-25:
 * - Report the right filename when the open fails.
 * - Use the input file name verbatim, don't add the extension
 *   (because it was added in upper case)
 * - Make the output extension lower case.
...
void mus2mid(unsigned char* data)
{
  FILE *midst;
  MUSheader MUSh;
  UBYTE *mus,*mid;
  ...
  if (data)
  {
    ...
    midst = fopen("DOOMSONG.MID", "wb");
    if (midst)
    {
      fwrite(mid,midlen,1,midst);
      fclose(midst);
    }

  //jff 8/3/98 use logical output routine
  //lprintf(LO_CONFIRM,"MUS file %s converted to MIDI file %s\n",musfile, "DOOMSONG.MID");
  FreeTracks(&mididata);
  free(mid);
  }
}
