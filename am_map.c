...
dboolean AM_Responder( event_t *ev )
   {
    ...
    if (ev->type == ev_keydown)
       {
        ...
        if (ev->data1 == key_map_zoomin)
           {
            mtof_zoommul = M_ZOOMIN;
            ftom_zoommul = M_ZOOMOUT;
           }
        else
        if (ev->data1 == key_map_zoomout)
           {
            mtof_zoommul = M_ZOOMOUT;
            ftom_zoommul = M_ZOOMIN;
           }
        else
        if (ev->data1 == key_map_gobig)
           {
            bigstate = !bigstate;
            if (bigstate)
               {
        ...
        if (ev->data1 == key_map_grid)
           {
            grid = !grid;
            plr->message = grid ? AMSTR_GRIDON : AMSTR_GRIDOFF;
           }
        else
        if (ev->data1 == key_map_mark)
           {
    ...
    if (ev->type == ev_keyup)
       {
        rc = false;
        if ((ev->data1 == key_map_right) || (ev->data1 == key_map_left))
           {
            if (!followplayer)
                m_paninc.x = 0;
           }
        else
        if ((ev->data1 == key_map_up) || (ev->data1 == key_map_down))
           {
            if (!followplayer)
                m_paninc.y = 0;
           }
        else
        if ((ev->data1 == key_map_zoomin) || (ev->data1 == key_map_zoomout))
           {
            mtof_zoommul = FRACUNIT;
            ftom_zoommul = FRACUNIT;
           }
...
//
// Draw OpenGL lines - what could be easier?
//
...
//
void GL_AM_drawMline( mline_t *ml,int color )
   {
    static fline_t fl;

    if (AM_clipMline(ml, &fl))
       {
        GL_AM_drawFline(&fl, color); // draws it on frame buffer using fb coords
       }
   }
...
void GL_AM_drawWalls(void)
{
	...
	if (cheating || (lines[i].flags & ML_MAPPED))
	{
	    ...
	    {
		GL_AM_drawMline(&l, WALLCOLORS+lightlev);
	    }
	    else
	    {
		if (lines[i].special == 39)
		{ // teleporters
		    GL_AM_drawMline(&l, WALLCOLORS+WALLRANGE/2);
		}
		else if (lines[i].flags & ML_SECRET) // secret door
		{
		    if (cheating) GL_AM_drawMline(&l, SECRETWALLCOLORS + lightlev);
		    else GL_AM_drawMline(&l, WALLCOLORS+lightlev);
		}
		    ...
		    GL_AM_drawMline(&l, CDWALLCOLORS+lightlev); // ceiling level change
		}
		else if (cheating) {
		    GL_AM_drawMline(&l, TSWALLCOLORS+lightlev);
		}
	    }
	}
...
void GL_AM_drawLineCharacter( mline_t*	lineguy, int lineguylines, fixed_t scale, angle_t angle, int color, fixed_t	x, fixed_t	y )
   {
    int		i;
    mline_t	l;
