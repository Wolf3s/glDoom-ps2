...
typedef enum
{
  ...
  netabort      // network game was aborted
} GameMode_t;
...
typedef struct
   {
    float  x;   // camera x position
    float  y;   // camera y position
    float  z;   // camera z position
    float  ox;  // camera rotation about x axis (looking up and down)
    float  oy;  // camera rotation about y axis (looking side to side)
    float  oz;  // camera rotation about z axis (rolling head side to side)
   }camera_t;
