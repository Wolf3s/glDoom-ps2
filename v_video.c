...
//
void V_DrawPatchOffsetBuff( int x, int y, unsigned char *buff, int bx, int by, int patchnum )
   { 
    int	      count;
    int	      col; 
    column_t *column; 
...
// Masks a column based masked pic to the screen. 
//
void V_DrawPatchBuff ( int x, int y, unsigned char *buff, patch_t* patch )
   { 
    int		         count;
    int		         col; 
    ...
    //desttop = buff-(patch->topoffset*(patch->width-patch->leftoffset));
    //txp = Transparent-(patch->topoffset*(patch->width-patch->leftoffset));
       ...
       { 
        ...
        while (column->topdelta != 0xff )
           { 
            ...
            //dest = desttop + column->topdelta*patch->width;
            //draw = txp + column->topdelta*patch->width;
            ...
            while (count--) 
                ...
                *dest = *source++;
                *draw = GLD_COLORED;
                //dest += (patch->width-patch->leftoffset);
                //draw += (patch->width-patch->leftoffset);
                //dest += patch->width;
                //draw += patch->width;
                dest += pwidth;
                draw += pwidth;
