...
dboolean G_DeferedPlayDemo_II (char* name)
   { 
...
void G_DoPlayDemo_II(void) 
   { 
    ...
    lfprintf("Playing demo II : %s\n", defdemoname);
    length = M_GetFileSize(defdemoname);
    demobuffer = malloc(length);
...
{ 
    ...
    if (demobuffer == NULL)
       {
        lfprintf("Demo %s is not in the WAD...\n", defdemoname);
        gameaction = ga_nothing;
        return;
       }
    ...
    if ( *demo_p++ != tversion)
    {
      lfprintf("Demo version : %d.%d Game version : %d.%d\n",
...
// G_TimeDemo 
//
dboolean G_TimeDemo_II (char* name) 
{ 	 
    static char demofilename[128];
    sprintf(demofilename, "%s.dem", name);
    if (Access(demofilename, 0) != 0)
    ...
    gameaction = ga_playdemo; 
    demotype = DEMO_II;
    lfprintf("Playing timedemo II %s -noblit %d -nodraw %d\n", name, noblit, nodrawers);
    return true;
} 
