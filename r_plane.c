...
void R_InitPlanes (void)
   {
    ...
    for (i = 0; i < MAXVISPLANES; i++)
       {
        visplanes[i].topbase = (unsigned short *)calloc(SCREENWIDTH+2,sizeof(unsigned short));
        visplanes[i].top = &visplanes[i].topbase[1];
        visplanes[i].bottombase = (unsigned short *)calloc(SCREENWIDTH+2,sizeof(unsigned short));
        visplanes[i].bottom = &visplanes[i].bottombase[1];
       }
...
visplane_t *R_FindPlane( fixed_t height, int picnum, int lightlevel )
   {
    ...
    memset (check->topbase,0xffff,sizeof(unsigned short)*(SCREENWIDTH+2)); // HIRES
  ...
  int		stop )
{
    ...
    memset (pl->topbase,0xffff,sizeof(unsigned short)*(SCREENWIDTH+2)); // HIRES
